...
class GaussianModel:
    ...
    def get_scaling(self):
        ...
        # first_column = self.scaling_activation(self._scaling[:, 0]).unsqueeze(1)
        # second_column = self.scaling_activation(self._scaling[:, 1]).unsqueeze(1)
        # third_column = torch.full((self._scaling.shape[0], 1), 1e-8, device=self._scaling.device)
        # scaling_result = torch.cat((first_column, second_column, third_column), dim=1)
        # return scaling_result
        #
        # third_column = torch.full((self._scaling.shape[0], 1), 5e-3, device=self._scaling.device)
        # scaling_result = torch.cat((third_column, third_column, third_column), dim=1)
        # return scaling_result
    ...
    def get_main_axis(self, view_cam):
        ...
        dir_global = rotation_matrices[..., 0]
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, cam_infos : int, spatial_lr_scale : float):
        ...
        fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors[...,0:1])).float().cuda())
    ...
    @torch.no_grad()
    def draw_ellipsoids(self, path, step, radius=1.5):
        # 获取椭圆的中心、旋转、缩放和颜色特征
        ...
        colors += 0.5
        colors = torch.relu(colors).clip(0, 1)  # 确保颜色在 [0, 1] 范围内
        # colors[torch.norm(grads, dim=-1) > 0.0002] = 1.0
        colors = colors.repeat(1, 3)
        # 生成椭圆网格
        def generate_ellipse_mesh(center, rotation, scale, color, light_position=np.array([10, 10, 10]),
                                  view_position=np.array([0, 0, 10])):
            """
            根据中心、旋转、缩放和颜色生成一个椭圆的网格，并添加光照效果。
            """
            # 创建一个单位球体
            ...
            # 应用缩放矩阵
            scale_matrix = np.diag([scale[0], scale[1], scale[2]])  # 只缩放 x 和 y 方向
        ...
        for i in range(len(xyzs)):
            center = xyzs[i].cpu().numpy()  # 中心位置
            rotation = rotations[i].cpu().numpy()  # 四元数
            # min_id = torch.argmin(scales[i])
            scale = [scales[i, 0].cpu().numpy(), scales[i, 1].cpu().numpy(), scales[i, 2].cpu().numpy()]  # 长轴和短轴
            # scale[min_id] += 0.002
            color = colors[i].cpu().numpy()  # 颜色特征归一化到 [0, 1]
    ...
    def max_opacity(self):
        opacities_new = self.inverse_opacity_activation(
            torch.max(self.get_opacity, torch.ones_like(self.get_opacity)))
        optimizable_tensors = self.replace_tensor_to_optimizer(opacities_new, "opacity")
        self._opacity = optimizable_tensors["opacity"]
        ...
        def save_pointcloud(path, xyzs, rotations, scales, features):
            xyzs = xyzs[:, None, :]
            rotations = rotations[:, None, :]
            scales = scales[:, None, :]

            # 计算长轴方向
            long_axis_dir = rotate_point_by_quaternion(
                rotations[:, 0, :], torch.tensor([[1, 0, 0]]).float().cuda()
            )  # 长轴方向为 (1, 0, 0)，旋转到全局坐标系
            long_axis_dir = long_axis_dir / torch.norm(long_axis_dir, dim=-1, keepdim=True)  # 单位化

            # 沿长轴方向采样点
            sample_positions = torch.linspace(-1, 1, sample_num).cuda()  # 沿长轴 [-0.5, 0.5] 采样
            sample_positions = sample_positions[:, None] * radius  # 映射到指定长度

            # 生成点云
            sampled_points = xyzs + sample_positions[None, :, :] * long_axis_dir[:, None, :] * \
                             torch.max(scales, dim=-1).values[..., None]  # [gs_num, sample_num, 3]
            sampled_points = sampled_points.reshape(-1, 3)  # 展平
            ...
            colors = colors[..., 0:1].repeat(1, 1, 3)
        ...
        save_pointcloud(
            os.path.join(path, f"line_step{step}.ply"), xyzs, rotations, scales, features
        )